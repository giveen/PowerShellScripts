...
$ADcomputers = Get-ADComputer -SearchBase "OU=xxxxx,DC=xxxxxx,DC=edu" -Filter "enabled -eq 'true'" | forEach-Object {$_.Name}
...
foreach ($computer in $PingCompList){
    ...
    {
        invoke-command -AsJob -ComputerName $computer.Address {wuauclt.exe /detectnow /resetauthorization /reportnow }
        invoke-command -AsJob -ComputerName $computer.Address {usoclient ScanInstallWait }
        write-host -foregroundcolor "Green" "WSUS is now checking in for computer: " $computer.Address
    }
    else{
        Write-host -foregroundcolor "Red" $computer.Address "is offline" 
    }   
} 